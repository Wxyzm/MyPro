...
{
    NSString *userid = nil;
    userid = [[NSUserDefaults standardUserDefaults] objectForKey:FYH_USER_ID];
    return userid;
}
...
+ (void)connectRongCloud
{
    ...
    if ([[UserPL shareManager] userIsLogin]) {
        // 连接融云服务器。
        [[RCIM sharedRCIM] connectWithToken:[[NSUserDefaults standardUserDefaults] objectForKey:FYH_RC_Token]     success:^(NSString *userId) {
            NSLog(@"登陆成功。当前登录的用户ID：%@", userId);
            [[NSUserDefaults standardUserDefaults]setObject:userId forKey:FYH_USER_ID];
