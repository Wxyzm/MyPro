...
@interface SearchResultViewController ()<SearchRTopViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UITextFieldDelegate>
...
-(UICollectionView *)collectionview
{
    if (_collectionview == nil)
    {
        ...
        _collectionview=[[UICollectionView alloc] initWithFrame:CGRectMake(0, 120, ScreenWidth, ScreenHeight-120) collectionViewLayout:flowLayout];
...
- (void)setupUI{
    ...
    _topView = [[SearchRTopView alloc]initWithFrame:CGRectMake(0, 80,ScreenWidth , 40)];
...
- (void)loadGoodsItems{
    ...
    NSDictionary *dic = @{
                          @"pageNum":[NSString stringWithFormat:@"%ld",_netModel.pageNum],
                          @"title":_netModel.title,
                          @"categoryId":_netModel.categoryId,
                          @"sort":_netModel.sort
                          };
[GoodsItemsPL GETGoodsIetmsWithDic:dic ReturnBlock:^(id returnValue) {
    NSDictionary *resultDic = returnValue[@"data"];
    self.loadArray = [ItemsModel mj_objectArrayWithKeyValuesArray:resultDic[@"itemList"]];
    if ( self.loadArray.count<=0) {
        [self showTextHud:@"没有更多数据了"];
    }
     [self loadSuccess];
    [self endLoading];
