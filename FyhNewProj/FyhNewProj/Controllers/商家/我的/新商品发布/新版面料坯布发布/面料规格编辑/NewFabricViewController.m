...
#import "NewFabricViewController.h"
...
@interface NewFabricViewController ()<FabricProUnitViewDelegate>
...
@implementation NewFabricViewController{
...
- (BOOL)makesureAllDatas{
    ...
    for (int i = 0; i<onArr.count; i++) {
        ...
        if (!model.stock||!model.price||!model.mineBuy) {
...
- (void)setDataWithkindTitleArr:(NSMutableArray *)kindTitleArr andstatusTitleArr:(NSMutableArray *)statusTitleArr andcolorTitleArr:(NSMutableArray *)colorTitleArr{
    ...
    if (kindTitleArr.count<=0||statusTitleArr.count<=0||![kindTitleArr containsObject:@"大货"]||colorTitleArr.count<=0) {
        [self.dataModel.dataArr removeAllObjects];
        self.batchView.dataArr =self.dataModel.dataArr;
        _IsHaVeSBat = NO;
    }else{
        _IsHaVeSBat = YES;
...
//- (void)combine:(NSMutableArray *)result data:(NSArray *)data curr:(int)currIndex count:(int)count andresultArr:(NSMutableArray *)resultArr{
//
//    if (currIndex == count) {
//
//        [resultArr addObject:[result mutableCopy]];
//        [result removeLastObject];
//
//    }else {
//        NSArray* array = [data objectAtIndex:currIndex];
