...
#import "NewAccessViewController.h"
#import "NewAccessView.h"
...
@interface NewAccessViewController ()<NewAccessViewDelegate>

@property (nonatomic,strong) NewAccessView   *fabView;               //顶部View
...
@implementation NewAccessViewController{
...
-(SampleColth *)colthView{
    if (!_colthView) {
        _colthView = [[SampleColth alloc] initWithFrame:CGRectMake(0, TOPVIEWBOOM, ScreenWidth, 104)];
    }
    return _colthView;
}
...
-(BatchSetView *)batchView{
    if (!_batchView) {
        _batchView = [[BatchSetView alloc] initWithFrame:CGRectMake(0, TOPVIEWBOOM +104, ScreenWidth, self.dataModel.dataArr.count *112 +51)];
...
- (void)initUI{
    ...
    _fabView = [[NewAccessView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, TOPVIEWBOOM)];
    ...
    self.colthView.nameLab.text = @"零售";
    if ([self.dataModel.kindArr containsObject:@"零售"]) {
...
- (void)respondToRightButtonClickEvent{
    ...
    [PublishGoodsPL batchspecificationWithInfoDic:setDic ReturnBlock:^(id returnValue) {
        ...
        if ([outArr containsObject:@"批发"]) {
            [outArr removeObject:@"批发"];
        }
...
- (void)setDataWithkindTitleArr:(NSMutableArray *)kindTitleArr andstatusTitleArr:(NSMutableArray *)statusTitleArr andcolorTitleArr:(NSMutableArray *)colorTitleArr{
    ...
    if (kindTitleArr.count<=0||statusTitleArr.count<=0||![kindTitleArr containsObject:@"批发"]||colorTitleArr.count<=0) {
    ...
    if (![kindTitleArr containsObject:@"零售"]||colorTitleArr.count<=0) {
        _IsHaVeSampleCloth = NO;
    }else{
        _IsHaVeSampleCloth = YES ;
    }
    
    if (colorTitleArr.count>0&&statusTitleArr.count>0&&[kindTitleArr containsObject:@"批发"])
    {
        NSMutableArray *colorArr = [NSMutableArray arrayWithCapacity:0];
        ...
        NSMutableArray* result = [NSMutableArray array];
        NSMutableArray* array_data = [NSMutableArray arrayWithObjects:
                                      [NSMutableArray arrayWithObject:@"批发"],
                                      statusTitleArr,
                                      colorArr,
                                      nil];
