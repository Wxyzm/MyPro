...
@interface FabricView ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic , strong) NSMutableArray *leixinArr;
...
- (void)setUp{
    ...
    NSMutableArray *leixinViewArr = [NSMutableArray arrayWithObjects:@"样卡",@"样布",@"大货", nil];
    _leixinArr = [NSMutableArray arrayWithCapacity:0];
    for (int i = 0; i<leixinViewArr.count; i++) {
        ...
        if (i == 2) {
            model.originId = 1000+i+1;
        }
        [_leixinArr addObject:model];
    }
    
    [_leixinView setTagWithTagArray:_leixinArr];
    
    _kindnameArr = [NSMutableArray arrayWithObjects:@"定制",@"自定义", nil];
    ...
    [_leixinView setDidselectStatusItemBlock:^(UnitModel *model) {
        ...
        if (weakself.itemIsEditComplete) {
            ...
            [alert  addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                if ([weakself.delegate respondsToSelector:@selector(didSelectedFabricViewRemoveListArrBtn)]) {
                    [weakself.delegate didSelectedFabricViewRemoveListArrBtn];
                }
                
                [weakself refreshViewFrame];
                model.on= !model.on;
                [weakself.kindView setTagWithTagArray:weakself.kindArr];
                [weakself.leixinView setTagWithTagArray:_leixinArr];
                [weakself refreshViewFrame];
                if (model.on) {
        ...
        if (weakself.itemIsEditComplete) {
            ...
            [alert  addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                if (model.originId !=1002) {
                    ...
                    if ([weakself.delegate respondsToSelector:@selector(didSelectedFabricViewAddBtnwithType:)]) {
                        [weakself.delegate didSelectedFabricViewAddBtnwithType:1];
                    }
                }
            }]];
            [alert  addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    [_kindView setDidDeleteItemBlock:^(UnitModel *model) {
        ...
        if (weakself.itemIsEditComplete) {
            ...
            [alert  addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                if ([weakself.delegate respondsToSelector:@selector(didSelectedFabricViewdeleteBtn)]) {
                    [weakself.delegate didSelectedFabricViewdeleteBtn];
                }
                if ([weakself.delegate respondsToSelector:@selector(didSelectedFabricViewRemoveListArrBtn)]) {
                    [weakself.delegate didSelectedFabricViewRemoveListArrBtn];
                }
...
- (void)goodslistBtnclick{
    ...
    if ([self.delegate respondsToSelector:@selector(didSelectedFabricViewGoodsList)]) {
        [self.delegate didSelectedFabricViewGoodsList];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == _unitArr.count ) {
        if ([self.delegate respondsToSelector:@selector(didSelectedFabricViewAddBtnwithType:)]) {
            [self.delegate didSelectedFabricViewAddBtnwithType:0];
        }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [self refreshViewFrame];
        if ([self.delegate respondsToSelector:@selector(didSelectedFabricViewdeleteBtn)]) {
            [self.delegate didSelectedFabricViewdeleteBtn];
        }
