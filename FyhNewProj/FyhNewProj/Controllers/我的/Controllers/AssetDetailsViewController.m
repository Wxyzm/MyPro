...
@interface AssetDetailsViewController ()<UITableViewDelegate,UITableViewDataSource>
...
[BankCardPL userGetbillrecordWithinfoDic:dic ReturnBlock:^(id returnValue) {
    NSLog(@"%@",returnValue);
    NSMutableArray *arr = returnValue[@"billRecords"];
    
    
    self.loadArray = [AssetDetailModel mj_objectArrayWithKeyValuesArray:arr];
...
- (void)loadSuccess
{
    ...
    for (AssetDetailModel *obj in self.dataArr.reverseObjectEnumerator) {
        if ([obj.amount floatValue]==0.00) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    AssDetailsCell *cell = [tableView dequeueReusableCellWithIdentifier:cellid];
    if (!cell) {
        cell = [[AssDetailsCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellid];
    }
