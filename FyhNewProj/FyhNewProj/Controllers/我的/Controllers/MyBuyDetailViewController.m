...
@interface MyBuyDetailViewController ()<SDCycleScrollViewDelegate>
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshtableview:)
                                                 name:@"HaveReceivePrice" object:nil];
...
[PublicWantPL UserLookHisNeedwithPricewithId:_needId WithReturnBlock:^(id returnValue) {
    NSLog(@"%@",returnValue);
    [self.view.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self initUIWithDic:returnValue[@"data"][@"purchasingNeed"]];
...
- (void)initUIWithDic:(NSDictionary *)dic{
    ...
    NSArray *imageURLStringsGroup =  dic[@"imageUrlList"];
    ...
    UILabel *seeLab = [BaseViewFactory labelWithFrame:CGRectZero textColor:UIColorFromRGB(0xaab2bd) font:APPFONT(13) textAligment:NSTextAlignmentRight andtext:[NSString stringWithFormat:@"浏览:%@",NULL_TO_NIL(dic[@"viewedCount"])?dic[@"viewedCount"]:@"0"]];
    ...
    NSArray *arr = dic[@"quotations"];
    if (arr.count >5) {
        for (int i = 0; i<5; i++) {
            [_priceArr addObject:arr[i]];
        }
    }else{
        [_priceArr addObjectsFromArray:arr];
    }
    ...
    [self createLabelWith:UIColorFromRGB(BlackColorValue) Font:APPFONT(15) WithSuper:_priceView Frame:CGRectMake(0, 13, ScreenWidth-20, 15) Alignment:NSTextAlignmentCenter Text:@"商家报价单"];
    for (int i = 0; i<_priceArr.count; i++) {
        NSDictionary *peiceDic = _priceArr[i];
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(13, 38+120 *i, ScreenWidth - 46, 110)];
        [_priceView addSubview:imageView];
        imageView.image = [UIImage imageNamed:@"block"];
        imageView.userInteractionEnabled = YES;
        
        UILabel *nameLab = [BaseViewFactory labelWithFrame:CGRectMake(25, 11, imageView.width, 12) textColor:UIColorFromRGB(BlackColorValue) font:APPFONT(12) textAligment:NSTextAlignmentLeft andtext:[NSString stringWithFormat:@"用户：%@",peiceDic[@"supplierInfo"]]];
    ...
    _lookBtn = [SubBtn buttonWithtitle:@"查看全部报价" backgroundColor:[UIColor clearColor] titlecolor:UIColorFromRGB(0x434a54) cornerRadius:12 andtarget:self action:@selector(lookBtnClick)];
    _lookBtn.layer.borderColor = UIColorFromRGB(0x434a54).CGColor;
    _lookBtn.layer.borderWidth = 1;
    [_priceView addSubview:_lookBtn];
    _lookBtn.titleLabel.font = APPFONT(11);
    _lookBtn.frame = CGRectMake((ScreenWidth-20)/2-40, 53+120 *_priceArr.count, 80, 24);
    _priceView.frame = CGRectMake(10, _originY, ScreenWidth - 20, _lookBtn.bottom +13);
    _isOpen = NO;
 ...
 */
- (void)lookBtnClick{
    if (_priceArr.count <= 0) {
        [self showTextHud:@"暂未有商家对此需求报价"];
        return;
    }
    
    MyBuyListViewController *buylistVc = [[MyBuyListViewController  alloc]init];
    buylistVc.listArr = _infoDic[@"quotations"];
    buylistVc.unitStr = _infoDic[@"unit"];
    buylistVc.infoDic = _infoDic;
...
- (void)acceptBtnClick:(SubBtn *)btn{
    ...
    NSDictionary *dic = _priceArr[btn.tag - 1000];
...
- (void)openBtnClick{
    ...
    if (!_isOpen) {
        [UIView animateWithDuration:0.1 animations:^{
            ...
            _unView.hidden = YES;
            _unView.frame = CGRectMake(0, _ViewOriginY, ScreenWidth, _unHeight);
            _priceView.frame = CGRectMake(10, _ViewOriginY, ScreenWidth - 20, _lookBtn.bottom +13);
            self.bgScrollView.contentSize = CGSizeMake(10, _priceView.bottom +20);
        }];
    ...
    else
    {
        [UIView animateWithDuration:0.1 animations:^{
            ...
            _unView.hidden = NO;
            _unView.frame = CGRectMake(0, _ViewOriginY, ScreenWidth, _unHeight);
            _priceView.frame = CGRectMake(10, _ViewOriginY + _unHeight +12 , ScreenWidth - 20, _lookBtn.bottom +13);
...
- (void)chatBtnClick:(SubBtn *)btn{
    NSDictionary *dic = _priceArr[btn.tag - 2000];
    NSString * status;
    if ([_infoDic[@"status"] intValue] == 1) {
        status = @"状态:审核中";
    }else if ([_infoDic[@"status"] intValue] == 3){
